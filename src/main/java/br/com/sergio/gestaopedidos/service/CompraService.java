...
public class CompraService {
    ...
    private ItemCompra criarItem(TipoCompra tipo,ItemCompraRequest request){
        ...
        else {Produto p=produtoService.buscarEntidadePorId(request.getReferenciaId());if(p.getTipoProduto()!=TipoProduto.PRODUTO_REVENDA)throw new BusinessException("Somente produtos de revenda podem ser incluídos nesta compra.");if(!Boolean.TRUE.equals(p.getAtivo()))throw new BusinessException("Este produto não está disponível para novas compras.");item.setProduto(p);item.setInsumo(null);item.setNomeHistorico(p.getNome());item.setUnidadeHistorica(p.getUnidadeVenda()==UnidadeVenda.UNIDADE?UnidadeMedida.UNIDADE:UnidadeMedida.QUILOGRAMA);}
