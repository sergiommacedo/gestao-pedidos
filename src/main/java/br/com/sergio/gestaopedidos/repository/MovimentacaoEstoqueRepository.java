...
public interface MovimentacaoEstoqueRepository extends JpaRepository<MovimentacaoEstoque,Long>{
 ...
 @EntityGraph(attributePaths={"insumo","itemCompra","itemCompra.compra"})@Query("""
 ...
 AND (:tipo IS NULL OR m.tipo=:tipo) AND (:compraId IS NULL OR m.itemCompra.compra.id=:compraId)
 ...
 @EntityGraph(attributePaths={"insumo","itemCompra","itemCompra.compra"})List<MovimentacaoEstoque> findTop20ByInsumoIdOrderByDataMovimentacaoDescIdDesc(Long insumoId);
 @EntityGraph(attributePaths={"insumo","itemCompra"})List<MovimentacaoEstoque> findByItemCompraCompraIdAndTipoOrderByIdAsc(Long compraId,TipoMovimentacaoEstoque tipo);
