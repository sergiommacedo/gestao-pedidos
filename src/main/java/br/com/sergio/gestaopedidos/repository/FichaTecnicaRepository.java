...
public interface FichaTecnicaRepository extends JpaRepository<FichaTecnica, Long> {
    ...
    @Query(value="SELECT COUNT(*) FROM produtos p WHERE p.ativo=true AND p.tipo_produto='PREPARACAO_PRODUZIDA' AND NOT EXISTS (SELECT 1 FROM fichas_tecnicas f WHERE f.produto_id=p.id)",nativeQuery=true) long contarProdutosProduzidosAtivosSemFicha();
    ...
    @Query(value = """
            ...
            WHERE p.tipo_produto='PREPARACAO_PRODUZIDA' AND (:produto = '' OR LOWER(p.nome) LIKE LOWER(CONCAT('%', :produto, '%')))
