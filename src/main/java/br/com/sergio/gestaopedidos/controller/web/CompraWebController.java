...
public class CompraWebController {
    ...
    @GetMapping("/itens/buscar")@ResponseBody public List<Map<String,Object>> buscarItens(@RequestParam TipoCompra tipo,@RequestParam(defaultValue="")String termo){if(tipo==TipoCompra.PRODUTO_REVENDA)return produtoService.buscarRevendaAtivosPorNome(termo).stream().map(p->itemBusca(p.id(),p.nome(),p.unidadeVenda()==UnidadeVenda.UNIDADE?UnidadeMedida.UNIDADE:UnidadeMedida.QUILOGRAMA,"Revenda")).toList();return insumoService.buscarAtivosPorNome(termo).stream().map(i->itemBusca(i.id(),i.nome(),i.unidadeMedida(),"Insumo")).toList();}

    private Map<String,Object> itemBusca(Long id,String nome,UnidadeMedida unidade,String categoria){Map<String,Object> m=new LinkedHashMap<>();m.put("id",id);m.put("nome",nome);m.put("unidade",unidade.name());m.put("unidadeDescricao",unidade.getDescricao());m.put("simbolo",unidade.getSimbolo());m.put("categoria",categoria);return m;}
    private CompraRequest request(CompraResponse c){return CompraRequest.builder().tipoCompra(c.tipoCompra()).dataCompra(c.dataCompra()).fornecedor(c.fornecedor()).observacao(c.observacao()).itens(c.itens().stream().map(i->ItemCompraRequest.builder().id(i.id()).referenciaId(i.referenciaId()).quantidade(i.quantidade()).valorTotalItem(i.valorTotalItem()).build()).toList()).build();}
    private List<ItemCompraResponse> itensFormulario(CompraRequest r){if(r.getItens()==null||r.getTipoCompra()==null)return List.of();List<ItemCompraResponse> itens=new ArrayList<>();for(ItemCompraRequest i:r.getItens()){if(i.getReferenciaId()==null)continue;try{String nome;UnidadeMedida unidade;if(r.getTipoCompra()==TipoCompra.INSUMO){var ref=insumoService.buscarPorId(i.getReferenciaId());nome=ref.nome();unidade=ref.unidadeMedida();}else{var ref=produtoService.buscarPorId(i.getReferenciaId());nome=ref.nome();unidade=ref.unidadeVenda()==UnidadeVenda.UNIDADE?UnidadeMedida.UNIDADE:UnidadeMedida.QUILOGRAMA;}BigDecimal custo=i.getQuantidade()!=null&&i.getQuantidade().signum()>0&&i.getValorTotalItem()!=null?i.getValorTotalItem().divide(i.getQuantidade(),6,RoundingMode.HALF_UP):BigDecimal.ZERO;itens.add(ItemCompraResponse.builder().id(i.getId()).referenciaId(i.getReferenciaId()).nomeHistorico(nome).categoria(r.getTipoCompra().getDescricao()).unidadeHistorica(unidade).quantidade(i.getQuantidade()).valorTotalItem(i.getValorTotalItem()).custoUnitario(custo).build());}catch(RuntimeException ignored){}}return itens;}
    private void preparar(Model model,boolean edicao,Long id,List<ItemCompraResponse> itens,boolean bloqueada){model.addAttribute("titulo",edicao?"Editar compra":"Nova compra");model.addAttribute("modoEdicao",edicao);model.addAttribute("compraId",id);model.addAttribute("itensFormulario",itens);model.addAttribute("financeiroBloqueado",bloqueada);model.addAttribute("tiposCompra",TipoCompra.values());}
}
