...
public class CompraInsumoWebController {
    ...
    @GetMapping("/{id}")public String detalhes(@PathVariable Long id,Model model){model.addAttribute("compra",compraService.buscarPorId(id));return"compras-insumos/detalhes";}
    @GetMapping("/{id}/editar")public String editar(@PathVariable Long id,Model model){var c=compraService.buscarPorId(id);model.addAttribute("compra",request(c));preparar(model,true,id,c.itens());return"compras-insumos/formulario";}
